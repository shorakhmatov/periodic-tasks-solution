...
func NewRouter(taskHandler *httphandlers.TaskHandler, docsHandler *swaggerdocs.Handler) *mux.Router {
	...
	// Periodic task management endpoints
	api.HandleFunc("/tasks/generate-recurring", taskHandler.GenerateRecurringTasks).Methods(http.MethodPost)
	api.HandleFunc("/tasks/templates", taskHandler.GetTemplateTasks).Methods(http.MethodGet)
	api.HandleFunc("/tasks/{id:[0-9]+}/children", taskHandler.GetChildTasks).Methods(http.MethodGet)
