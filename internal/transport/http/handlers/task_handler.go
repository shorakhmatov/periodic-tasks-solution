...
type TaskHandler struct {
	...
	recurringService *taskusecase.RecurringService
}

func NewTaskHandler(usecase taskusecase.Usecase, recurringService *taskusecase.RecurringService) *TaskHandler {
	return &TaskHandler{
		usecase:         usecase,
...
func (h *TaskHandler) GenerateRecurringTasks(w http.ResponseWriter, r *http.Request) {
	...
	response := make([]taskDTO, 0, len(generatedTasks))
...
func (h *TaskHandler) GetTemplateTasks(w http.ResponseWriter, r *http.Request) {
	...
	response := make([]taskDTO, 0, len(templateTasks))
	for i := range templateTasks {
...
func (h *TaskHandler) GetChildTasks(w http.ResponseWriter, r *http.Request) {
	id, err := getIDFromRequest(r)
	if err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	...
	response := make([]taskDTO, 0, len(childTasks))
	for i := range childTasks {
		response = append(response, newTaskDTO(&childTasks[i]))
	}
