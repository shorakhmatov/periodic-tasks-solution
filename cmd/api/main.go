...
func main() {
	...
	recurringService := task.NewRecurringService(taskRepo)
	scheduler := task.NewScheduler(taskRepo, recurringService)
	taskHandler := httphandlers.NewTaskHandler(taskUsecase, recurringService)
